...
namespace SQIndustryThree.DAL
{
    public class BillApprovalDAL
    {
        ...
        public DataTable POUniqueNumber(int PONumber)
        {
            try
            {
                ...
                string sql = "Select PONo from Bill_PurchaseOrderMaster where PONo = @PONumber";
        ...
        public DataTable QuantityCheck(long PODetailsKey)
        {
            try
            {
                DataTable dt = new DataTable();
                ...
                string sql = "SELECT POQty, BalancePIQty FROM [Bill_PurchaseOrderDetails] where PODetailsKey = @PODetailsKey";

                SqlCommand cmd = new SqlCommand(sql, conn);
                cmd.CommandType = CommandType.Text;
                //cmd.Parameters.Add("@cheque", SqlDbType.Int).Value = supplierId;
                cmd.Parameters.Add("@PODetailsKey", SqlDbType.Int).Value = PODetailsKey;
                SqlDataAdapter adpt = new SqlDataAdapter(cmd);
                adpt.Fill(dt);
                return dt;
            }
        ...
        public ResultResponse PODetailsInsert(long POKey, int PONumber, string Article, string ArticleCode, string Currency, string Color,
            string Size, string UOM, decimal POQty, float Rate, decimal POValue)
        {
            try
            {
                ...
                aParameters.Add(new SqlParameter("@Currency", Currency));
        ...
        public ResultResponse PODetailsTempDataInsert(long POKey, int PONumber, string Article, string ArticleCode, string Currency, string Color,
            string Size, string UOM, decimal POQty, float Rate, decimal POValue)
        {
            try
            {
        ...
        public DataTable PODetailsInsert()
        {
            //int acc = 0;
            DataTable dt = new DataTable();
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("sp_SavePODetailsRep", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable DertmentList()
        {
            ...
            SqlCommand cmd = new SqlCommand("sp_Departmentlist", conn);
            cmd.CommandType = CommandType.StoredProcedure;
            SqlDataAdapter adpt = new SqlDataAdapter(cmd);
            adpt.Fill(dt);
            return dt;
        ...
        public DataTable InvoiceSubcategoryList(int InvoiceType/*,int departmentID*/)
        {
            DataTable dt = new DataTable();
            if (conn.State == 0)
            {
        ...
        public DataTable PODetailsList(string POList)
        {
            DataTable dt = new DataTable();
            if (conn.State == 0)
            {
            ...
            SqlCommand cmd = new SqlCommand("sp_PODetailsList", conn);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.Add("@POList", SqlDbType.NVarChar).Value = POList;
            SqlDataAdapter adpt = new SqlDataAdapter(cmd);
            adpt.Fill(dt);
            return dt;
        }
        ...
        public DataTable InvoiceTypeWiseApproverList( int businessID, int invoiceType, int subcategoryId/*,int department*/)
        {
            DataTable dt = new DataTable();
            if (conn.State == 0)
            {
            ...
            //cmd.Parameters.Add("@departmentID", SqlDbType.Int).Value = department;
    ...
    string comment, string fileName, string filePath, int created_by)
        ...
        public BillRequestMaster GetBillInforamtion(int masterId, int userId)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    billrequest.GLID = Convert.ToInt32(dr["GLID"]);
                    billrequest.LedgerName = dr["LedgerName"].ToString();
          ...
          )
        {
            try
            {
                ...
                return this.accessManager.SaveData("sp_billApproveOrreject", new List<SqlParameter>()
                {
                  ...
                  new SqlParameter("@CompletionPercent", CompletionPercent),
                  new SqlParameter("@CompletionRemarks", CompletionRemarks),
                  new SqlParameter("@GLID", glid)
                });
            }
            catch (Exception ex)
            {
        ...
        //public bool UpdateBillMaster(int invoiceKey, decimal totalQty, decimal totalValue, decimal totalPaid, double discountPercent,
        //    decimal discountAmt, decimal totalAmount, double vat, decimal vatAmt, decimal netValue, double adjustmentPercent, decimal adjustmentAmt, double retaintionPercent, decimal retaintionAmt, decimal total_adjustment_amt,
        //    string tax, string taxAmt, int userID, List<DocumentCenterFileUpload> documentCenter)
        //{
        //    bool result = true;
        //    try
        //    {
        //        var docFilelist = new JavaScriptSerializer().Serialize(documentCenter);
        ...
        //        aList.Add(new SqlParameter("@tax", tax));
        //        aList.Add(new SqlParameter("@taxAmt", taxAmt));
        //        aList.Add(new SqlParameter("@docFileJson", docFilelist));
        //        aList.Add(new SqlParameter("@createdBy", userID));
        //        result = accessManager.UpdateData("sp_UpdateBillMaster", aList);
        //        return result;
        //    }
        ...
        //public int DeleteDocumentCenterFileFromDatatbase(int FileID)
        //{
        //    int i = 0;
        //    if (conn.State == 0)
        //    {
        //        conn.Open();
        //    }
        //    SqlCommand cmd = new SqlCommand("sp_DeleteDocumentCenterFile", conn);
        //    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable GeneralLedgerList()
        {
            DataTable dt = new DataTable();
            if (conn.State == 0)
            {
            ...
            SqlCommand cmd = new SqlCommand("sp_GeneralLedgerList", conn);
            cmd.CommandType = CommandType.StoredProcedure;
            //cmd.Parameters.Add("@unitId", SqlDbType.Int).Value = unitId;
            SqlDataAdapter adpt = new SqlDataAdapter(cmd);
            adpt.Fill(dt);
        ...
        public DataTable BankDetailsList()
        {
            ...
            SqlCommand cmd = new SqlCommand("sp_BankDetailsList", conn);
            cmd.CommandType = CommandType.StoredProcedure;
            //cmd.Parameters.Add("@unitId", SqlDbType.Int).Value = unitId;
            SqlDataAdapter adpt = new SqlDataAdapter(cmd);
            adpt.Fill(dt);
        ...
        public DataTable SupplierWiseBankInfo(int supplierID)
        {
            try
            {
                DataTable dt = new DataTable();
                ...
                SqlCommand cmd = new SqlCommand("sp_SupplierWiseBankInfo", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@SupplierID", SqlDbType.Int).Value = supplierID;
                //cmd.Parameters.Add("@UnitID", SqlDbType.NVarChar).Value = UnitId;
                SqlDataAdapter adpt = new SqlDataAdapter(cmd);
        ...
        public DataTable AllocationPODetails(AllocationPODetails allocationPODetails, int UserId)
        {
            try
            {
                DataTable dt = new DataTable();
                ...
                allocationPODetails.CreatedBy = UserId;

                SqlCommand cmd = new SqlCommand("sp_AllocationPODetails", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@SupplierId", SqlDbType.Int).Value = allocationPODetails.SupplierId;
                cmd.Parameters.Add("@InvoiceKey", SqlDbType.Int).Value = allocationPODetails.InvoiceKey;
                cmd.Parameters.Add("@InvoiceNo", SqlDbType.NVarChar).Value = allocationPODetails.InvoiceNo;
                cmd.Parameters.Add("@PONo", SqlDbType.Int).Value = allocationPODetails.PONo;
                cmd.Parameters.Add("@POQty", SqlDbType.Decimal).Value = allocationPODetails.POQty;
                cmd.Parameters.Add("@POValue", SqlDbType.Decimal).Value = allocationPODetails.POValue;
                cmd.Parameters.Add("@ApprovedValue", SqlDbType.Decimal).Value = allocationPODetails.ApprovedValue;
                cmd.Parameters.Add("@PaidAmount", SqlDbType.Decimal).Value = allocationPODetails.PaidAmount;
                cmd.Parameters.Add("@OutstandingAmount", SqlDbType.Decimal).Value = allocationPODetails.OutstandingAmount;
                cmd.Parameters.Add("@PaymentValue", SqlDbType.Decimal).Value = allocationPODetails.PaymentValue;
                cmd.Parameters.Add("@CreatedBy", SqlDbType.Int).Value = allocationPODetails.CreatedBy;
        ...
        public ResultResponse AddBankDetails(BankDetails data)
        {
            try
            {
                using (IDbConnection cnn = (IDbConnection)new SqlConnection(this.connStr))
                {
                    ...
                    dynamicParameters.Add("@BankName", (object)data.BankName);
                    dynamicParameters.Add("@IFSCorSWIFTCode", (object)data.IFSCorSWIFTCode);
                    dynamicParameters.Add("@AccountType", (object)data.AccountType);
                    dynamicParameters.Add("@AccountNumber", (object)data.AccountNumber);
                    dynamicParameters.Add("@Branch", (object)data.Branch);
                    dynamicParameters.Add("@BeneficiaryName", (object)data.BeneficiaryName);
                    dynamicParameters.Add("@SupplierId", (object)data.SupplierId);

                    int num = cnn.Execute("sp_AddBankDetails", (object)dynamicParameters, commandType: new CommandType?(CommandType.StoredProcedure));
        ...
        public ResultResponse EditBankDetails(BankDetails data)
        {
            try
            {
                using (IDbConnection cnn = (IDbConnection)new SqlConnection(this.connStr))
                {
                    if (cnn.State == ConnectionState.Closed)
                        cnn.Open();
                    DynamicParameters dynamicParameters = new DynamicParameters();
                    dynamicParameters.Add("@BankDetailsId", (object)data.BankDetailsId);
                    dynamicParameters.Add("@BankName", (object)data.BankName);
                    dynamicParameters.Add("@IFSCorSWIFTCode", (object)data.IFSCorSWIFTCode);
                    dynamicParameters.Add("@AccountType", (object)data.AccountType);
                    dynamicParameters.Add("@AccountNumber", (object)data.AccountNumber);
                    ...
                    int num = cnn.Execute("sp_EditBankDetails", (object)dynamicParameters, commandType: new CommandType?(CommandType.StoredProcedure));
                    return new ResultResponse()
                    {
                        pk = num,
                        isSuccess = true
        ...
        public dynamic GetAlllSuppliersForBankDetails()
        {
            try
            {
                accessManager.SqlConnectionOpen(DataBase.SQQeye);
                List<SqlParameter> aList = new List<SqlParameter>();
                SqlDataReader dr = accessManager.GetSqlDataReader("sp_LoadSuppliersForBankDetails", aList);
        ...
        public BankDetails GetBandDetailsForEdit(int id)
        {
            try
            {
                accessManager.SqlConnectionOpen(DataBase.SQQeye);
                ...
                SqlDataReader dr = accessManager.GetSqlDataReader("sp_BankDetailsForEditPage", aList);
                var bankDetails = new BankDetails();

                if (dr.Read())
                {
                    bankDetails.BankDetailsId = (int)dr["BankDetailsId"];
                    bankDetails.BankName = dr["BankName"].ToString();
                    bankDetails.IFSCorSWIFTCode = dr["IFSCorSWIFTCode"].ToString();
                    bankDetails.AccountType = dr["AccountType"].ToString();
                    bankDetails.AccountNumber = dr["AccountNumber"].ToString();
                    bankDetails.Branch = dr["Branch"].ToString();
                    bankDetails.BeneficiaryName = dr["BeneficiaryName"].ToString();
                    bankDetails.SupplierId = (int)dr["SupplierId"];
                }
