...
namespace SQIndustryThree.Controllers
{
    public class BillApprovalController : Controller
    {
                ...
                if (POcheck.Rows.Count == 0)
                {
                    ...
                    foreach (var item in poNumber)
                    {
                        ...
                        result = primarykey.pk;
                           ...
                           var details = billAprrovalPoDetails.Where(s => s.PONumber == item.PONumber).ToList();
                            foreach (var data in details)
                            {
                            //var dataVar = data;
                                var PODetailsInsert = billDal.PODetailsTempDataInsert(Convert.ToInt64(result), data.PONumber, data.ArticleName, data.ArticleCode, data.Currency, data.ColorName, data.SizeName, data.UOM, data.POQty, data.Rate, data.PoValue);
                            }
                    }

                    var parmanentData = billDal.PODetailsInsert();
        ...
        //    decimal totalQty, decimal totalValue,
        //    string discountPercent, string discountAmt, string totalAmount, double adjustmentPercent, decimal adjustmentAmt, double retaintionPercent, decimal retaintionAmt, decimal total_adjustment_amt,
        //     string vat, string vatAmt, string tax, string taxAmt, string netValue, List<DocumentCenterFileUpload> documentCenter)
        //{
        //    if (Session["SQuserId"] == null)
        //    {
        //        return RedirectToAction("Index", "Account");
        ...
        //            master = billDal.UpdateBillMaster(inCon, totalQty, totalValue, tCon, disPerCon, discountAmtCon, tAmtCon, vCon, vAmtCon, vNetCon, adjustmentPercent, adjustmentAmt, retaintionPercent, retaintionAmt, total_adjustment_amt, tax, taxAmt, userID, documentCenter);
        //        }
        ...
        public ActionResult CheckQty(int poDetailsKey, decimal updateQty)
        {
            bool status = false;
            if (Session["SQuserId"] == null)
            {
            ...
            if (!string.IsNullOrEmpty(poDetailsKey.ToString()))
            {
                chequeNo = billDal.QuantityCheck(poDetailsKey);

                if (chequeNo.Rows.Count > 0)
                {
                    DataRow row = chequeNo.Rows[0];

                    decimal poQty = Convert.ToDecimal(row["POQty"]);

                    decimal balanceQty = Convert.ToDecimal(row["BalancePIQty"]);

                    if (balanceQty + poQty < updateQty )
                    {
                        status = true;
                    }
                    else
        ...
        public ActionResult DepartmentList()
        {
            ...
            try
            {
                ...
                DataTable dt = billDal.DertmentList();
                //   List<Dictionary<string, object>>
                List<Dictionary<string, object>> _List = _BasicUtilities.GetTableRows(dt);
        ...
        public ActionResult InvoiceSubCategoryList(int invoiceType/*,int departmentID*/)
        {
            if (Session["SQuserId"] == null)
            {
                return RedirectToAction("Index", "Account");
            ...
            try
            {
                ...
                {
                    dt = billDal.InvoiceSubcategoryList(invoiceType /*, departmentID*/);
                }
        ...
        public ActionResult ApproverList(int businessUnitId, int invoiceType, int subcategory/*,int department*/)
        {
            if (Session["SQuserId"] == null)
            {
                return RedirectToAction("Index", "Account");
            ...
            try
            {
                ...
                DataTable dt = billDal.InvoiceTypeWiseApproverList(businessUnitId,invoiceType, subcategory /*, department*/);
                //   List<Dictionary<string, object>>
                List<Dictionary<string, object>> _List = _BasicUtilities.GetTableRows(dt);
        ...
        //[HttpPost]
        //public ActionResult DeleteDocumentCenterFileFromDatatbase(int fileId, string fileName)
        //{
        //    bool result = false;
        //    try
        //    {
        //        System.IO.File.Delete(Server.MapPath("~/DocumentCenter/") + fileName);
        //        billDal.DeleteDocumentCenterFileFromDatatbase(fileId);
        ...
        public ActionResult BillApproveOrReject(string CommentText, int Progress, int RequestorId, string invoiceDetailKey, string verifyQty, string verifyValue,
            List<GRNFileUploadDetails> gRNFiles, List<POFileUploadDetails> poFiles, List<BillFileUploadDetails> billFiles, List<DocumentCenterFileUpload> billDocList, 
            int ProcurementUserId, int CapexInfoId, int CostCenterId, double CompletionPercent, string CompletionRemarks, int glid)
        {
            if (base.Session["SQuserId"] == null)
            {
                return base.RedirectToAction("Index", "Account");
            ...
            var approvalInfo = this.billDal.BillApproveOrReject(Progress, CommentText, userID, RequestorId, gRNFiles,poFiles,billFiles, billDocList, ProcurementUserId, CapexInfoId, CostCenterId, CompletionPercent, CompletionRemarks, glid);
        ...
        public ActionResult GeneralLedgerList()
        {
            ...
            try
            {
                ...
                DataTable dt = billDal.GeneralLedgerList();
                //   List<Dictionary<string, object>>
                List<Dictionary<string, object>> _List = _BasicUtilities.GetTableRows(dt);
        ...
        public ActionResult PODetailsList(string POList)
        {
            if (Session["SQuserId"] == null)
            {
                return RedirectToAction("Index", "Account");
            ...
            try
            {
                DataTable dt = billDal.PODetailsList(POList);
                //   List<Dictionary<string, object>>
                List<Dictionary<string, object>> _List = _BasicUtilities.GetTableRows(dt);
        ...
        public ActionResult BankDetailsList(string ViewName)
        {
            if (Session["SQuserId"] == null)
            {
                return RedirectToAction("Index", "Account");
            ...
            try
            {
                DataTable dt = billDal.BankDetailsList();
                //   List<Dictionary<string, object>>
                List<Dictionary<string, object>> _List = _BasicUtilities.GetTableRows(dt);

                ViewBag.BankDetails = _List;
        ...
        public ActionResult SupplierWiseBankInfo(int supplierID)
        {
            if (Session["SQuserId"] == null)
            {
                return RedirectToAction("Index", "Account");
            ...
            try
            {
                ...
                DataTable dt = billDal.SupplierWiseBankInfo(supplierID);
                //   List<Dictionary<string, object>>
                List<Dictionary<string, object>> _List = _BasicUtilities.GetTableRows(dt);
        ...
        public ActionResult AllocationPODetails(List<AllocationPODetails> allocationPODetails)
        {
            if (Session["SQuserId"] == null)
            {
                return RedirectToAction("Index", "Account");
            ...
            try
            {
                ...
                List<Dictionary<string, object>> _List = null;

                //var result = JsonConvert.DeserializeObject<AllocationPODetails>(allocationPODetails);
                if (allocationPODetails.Count > 0)
                {
                    foreach (var item in allocationPODetails)
                    {
                       // Dictionary<string, object> _result = null;
                        DataTable dt = billDal.AllocationPODetails(item, userId);


                        //   List<Dictionary<string, object>>
                      var  _result = dt.AsEnumerable().FirstOrDefault();
        ...
        public ActionResult EditBankDetails(int id)
        {
            ...
            ViewBag.BandDetailsId = id;
            return View();
        }

        [HttpPost]
        public ActionResult AddBankDetailsEntry(BankDetails data)
        {
            return Json(billDal.AddBankDetails(data), JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public ActionResult EditBankDetailsEntry(BankDetails data)
        {
            return Json(billDal.EditBankDetails(data), JsonRequestBehavior.AllowGet);
        }

        public ActionResult AllSuppliersForBankDetails()
        {
            return Json(billDal.GetAlllSuppliersForBankDetails(), JsonRequestBehavior.AllowGet);
        }

        public ActionResult BandDetailsForEdit(int id)
        {
            return Json(billDal.GetBandDetailsForEdit(id), JsonRequestBehavior.AllowGet);
        }
